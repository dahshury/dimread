# Feature-Sliced Design Agent Skill (v2.1)

This skill helps coding agents (Claude Code, Codex, and other agent harnesses) apply — and mechanically **enforce** — the Feature-Sliced Design (FSD) v2.1 architecture methodology on frontend projects.
...
- **NEW**: Bundled deterministic conformance checker (`check-fsd.mjs`) — 8 ERROR rules (always-genuine violations the agent fixes autonomously) + 6 REVIEW rules (flagged for agent judgment), with `fsd.config.json` for layout variants, acknowledgments, and documented exceptions
- Steiger architecture linter integration (CI alternative for strictly standard layouts)

## Bundled Conformance Checker (`check-fsd.mjs`)

Zero-dependency (Node >= 18) deterministic FSD checker, tuned against production codebases (980 and 3,335 files) until every ERROR finding is a genuine violation:

```bash
node check-fsd.mjs <srcDir>            # human-readable report
node check-fsd.mjs <srcDir> --json     # machine-readable (for agents)
node check-fsd.mjs <srcDir> --strict   # CI: fail on unacknowledged REVIEW flags too
```

- **8 ERROR rules** (`FSD-E1`–`E8`): upward imports, same-layer cross-slice imports, deep imports bypassing public APIs, `@x` misuse, missing slice index, deprecated `processes/`, layer-level public APIs, loose files on layer roots. Deterministic — agents fix these autonomously.
- **6 REVIEW rules** (`FSD-R1`–`R6`): single-consumer slices ("pages first" fold-back candidates), god slices, unknown top-level dirs, unrecognized shared segments, `@x` outside entities, empty/dead slices. Accurate signals flagged for agent/human judgment.
- Understands layout variants out of the box: `views`/`screens` as pages, app-tier `entries/`, root entry modules (`sw.ts`, generated bindings), FSD slice groups (one nesting level), test scaffolding excluded.
- Tokenizer-based import extraction: immune to imports inside comments, strings, and template literals; catches static, side-effect, dynamic, `export … from`, and type-position imports via aliases or relative paths.
- Project config via `fsd.config.json` (aliases, layer aliases, thresholds, documented `exceptions` and `acknowledgedReviews`); `// fsd-ignore` per-line suppression.

The full agent workflow (scan → fix errors autonomously → review flags → re-run until clean) is defined in `SKILL.md` under **Enforcement Workflow**.

## Installation

### Claude Code

Copy this folder to `~/.claude/skills/feature-sliced-design/` (or symlink it). Invoke with `/feature-sliced-design`.
...
- Restart your agent (Claude Code, Codex, ...).
