; Custom NSIS template with portable mode support.
...
;
; The installer is PORTABLE BY DEFAULT (.onInit forces PortableMode=1): running
; the exe extracts a self-contained folder with a `portable` marker file and a
; `Data/` directory, and skips registry writes, shortcuts, and the uninstaller.
; The install-type page still lets the user pick a Normal installation.
;
; The portable marker content is "${PRODUCTNAME} Portable Mode" — it must match
; PORTABLE_MARKER_MAGIC in src-tauri/src/portable.rs. If you rename the product,
; update both.
;
...
!else
  ...
  ; A wider LZMA history window improves the download size on payloads beyond
  ; a few MiB while keeping installer memory use modest on every supported
  ; machine (NSIS defaults to 8 MiB).
  !if "{{compression}}" == "lzma"
...
Function .onInit
  ...
  ; A previous portable install is recognized by its marker file content so an
  ; updater run never converts a portable install into a normal one.
  ${If} $PortableMode <> 1
  ${AndIf} $UpdateMode = 1
  ${AndIf} ${FileExists} "$INSTDIR\portable"
    ...
    FileRead $1 $2
    FileClose $1
    ${If} $2 == "${PRODUCTNAME} Portable Mode"
      StrCpy $PortableMode 1
    ${EndIf}
  ${EndIf}
...
Section Install
  ...
  ${If} $PortableMode = 1
    ...
    FileWrite $0 "${PRODUCTNAME} Portable Mode"
    FileClose $0
