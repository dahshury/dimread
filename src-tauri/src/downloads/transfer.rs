...
/// Cooperative pause/cancel flags polled by `transfer_url` on every chunk. Each download
/// handle embeds one of these instead of re-declaring an identical `{ paused, cancelled }`
/// pair plus its `TransferControl` impl.
///
