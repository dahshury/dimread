...
export interface SidebarLink {
	/** Uppercase micro-label rendered above this link, naming the group this
	 *  link starts. Collapsed mode preserves the slot as a hairline separator. */
	groupLabel?: string;
	icon: IconSvgElement;
	key: string;
	/** Extra searchable text (the tab's setting names) so the sidebar search
	 *  surfaces a tab by its contents, not just its label. */
...
const COLLAPSE_STORAGE_KEY = "starter:settings-sidebar-collapsed";
...
/**
 * The settings shell's left rail (ported from WinSTT): searchable,
 * collapsible tab list with grouped micro-labels and a top drag strip for
			...
			{collapsed ? (
				...
				<div className="settings-sidebar-header relative flex h-20 shrink-0 items-center gap-2 px-5 pt-6 pb-3">
					...
					>
						...
						>
							{/* Substrate 4 so the shared TextField lifts to surface-5 — the
							    same raised plate as the active sidebar tab. */}
						...
						{visibleLinks.length === 0 ? (
							...
							visibleLinks.map((link) => {
								const tab = (
									<Tabs.Tab
										className={cn(
											// No `bg-transparent` here: that utility would beat the
											// `.settings-sidebar-tab` background rules regardless of
											// specificity (layer order), so active/hover fills would
											// never paint.
